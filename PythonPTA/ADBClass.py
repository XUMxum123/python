...
class ADBClass:
	...
	def __init__(self, fileName):
		...
		print(self.getCurrentTime() + self.TAG + "ADBClass init")
	...
	def getPtalogStoreFile(self):
		ptalogFile = self.getRootDirectory() + self.ptalog_path + str(self.fileName) \
						+ self.ptalog_suffix
		return ptalogFile

	def executeCommand_adb_devices(self):
		command_adb_devices = "adb devices"
		print(self.getCurrentTime() + self.TAG + "execute command => " + command_adb_devices)
		try:
	...
	def executeCommand_adb_system_info(self):
		...
		print(self.getCurrentTime() + self.TAG + "execute command => " + command_adb_system_info)
		try:
	...
	def executeCommand_adb_logcat_c(self):
		...
		print(self.getCurrentTime() + self.TAG + "execute command => " + command_adb_logcat_c)
		try:
	...
	def executeCommand_adb_logcat_s(self, filter):
		...
		print(self.getCurrentTime() + self.TAG + "execute command => " + command_adb_logcat_s)
	...
	def executeCommand_grab_adb_log_v_time(self, deviceSerialNumber):
		...
		print(self.getCurrentTime() + self.TAG + "execute command => " + command_grab_adb_log_v_time)
	...
	def executeCommand_adb_kill_server(self):
		...
		print(self.getCurrentTime() + self.TAG + "execute command => " + command_adb_kill_server)
		try:
	...
	def executeCommand_adb_start_server(self):
		...
		print(self.getCurrentTime() + self.TAG + "execute command => " + command_adb_start_server)
		try:
	...
	def startGrabAdbLog(self):
		...
		if deviceSerialNumber == "":
			print(self.getCurrentTime() + self.TAG + "not find devices, safe exit python script")
			sys.exit(1)
		else:
			print(self.getCurrentTime() + self.TAG + "deviceSerialNumber: " + deviceSerialNumber)
		# clear adb log cache
		self.executeCommand_adb_logcat_c()
		print(self.getCurrentTime() + self.TAG + "wait 5 seconds, than start grad adb log")
		time.sleep(5)
		self.executeCommand_grab_adb_log_v_time(deviceSerialNumber)

	def stopGrabAdbLog(self):
		print(self.getCurrentTime() + self.TAG + "wait 10 seconds, than stop grab adb log")
		time.sleep(10)    
		self.executeCommand_adb_kill_server()  
		print(self.getCurrentTime() + self.TAG + "wait 5 seconds, than start server")
		time.sleep(5) 
		self.executeCommand_adb_start_server() 
		print(self.getCurrentTime() + self.TAG + "sleep 5 seconds, than adb devices")
		time.sleep(5) 
		deviceSerialNumber = self.getDeviceSerialNumber()
		if deviceSerialNumber == "":
			print(self.getCurrentTime() + self.TAG + "not find devices, safe exit python script")
	...
	def getDeviceSerialNumber(self):
		...
		if devicesOutputLen <= 29:
			print(self.getCurrentTime() + self.TAG + "not find any devices: " + str(devicesOutputLen) + " , than safe exit python script")
			return str(deviceSerialNumber)
			sys.exit(1)
		deviceInfo = devicesOutput.split("\r\n")
		len_deviceInfo = len(deviceInfo)
		if deviceInfo[1] == "":
			print(self.getCurrentTime() + self.TAG + "not find any devices")
			return str(deviceSerialNumber)
		elif deviceInfo[1] != "":
			print(self.getCurrentTime() + self.TAG + "find devices count: " + str(len_deviceInfo - 3))
			for i in range(1, (len_deviceInfo - 2)):
				dev = deviceInfo[i].split("\t")
				if len(dev) > 0:
					deviceSerialNumber = dev[0]
					print(self.getCurrentTime() + self.TAG + "device " + str(i) + " serial number: " + str(deviceSerialNumber))
					return str(deviceSerialNumber)
				else:
					print(self.getCurrentTime() + self.TAG + "deviceInfo " + str(i) + " len is 0")
					return str(deviceSerialNumber)
		else:
			print(self.getCurrentTime() + self.TAG + "deviceInfo none")
			return str(deviceSerialNumber)
	...
	def connectDevice(self):
		try:  
			...
			print(self.getCurrentTime() + self.TAG + "Device Connect Fail: " + e) 
	...
	def getAndroidVersion(self):  
		try:  
				...
				print (self.getCurrentTime() + self.TAG + "Get Android Version: " + e)
	...
	def getDeviceName(self):    
		try:  
				...
				print (self.getCurrentTime() + self.TAG + "Get Device Name: " + e)